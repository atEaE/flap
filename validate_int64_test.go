...
func TestInt64ValidatorRequired(t *testing.T) {
	// setup
	testcases := []struct {
		name  string
		value *int64
		...
		{name: "case1", value: int64Ptr(1), want: nil},
		{name: "case2", value: int64Ptr(213), want: nil},
	...
	for _, tc := range testcases {
		// act
		v := valigo.New()
		v.Int64VarP(tc.value, tc.name).Required()
...
func TestInt64ValidatorMax(t *testing.T) {
	// setup
	testcases := []struct {
		name  string
		value int64
	...
	for _, tc := range testcases {
		// act
		v := valigo.New()
		v.Int64Var(tc.value, tc.name).Max(10)
...
func TestInt64ValidatorMin(t *testing.T) {
	// setup
	testcases := []struct {
		name  string
		value int64
	...
	for _, tc := range testcases {
		// act
		v := valigo.New()
		v.Int64Var(tc.value, tc.name).Min(5)
